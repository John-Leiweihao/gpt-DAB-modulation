...
def PINN(Vin, Vref, P_required, modulation):
    ...
    if modulation == "EPS":
        ...
        ipp1, P_predicted1, pred1, inputs1, ZVS1, ZCS1, penalty1 = obj_func(optimal_x1[None], model_implicit_PINN,P_required, Vin, Vref, with_ZVS=True, modulation="EPS1",return_all=True,threshold_ZVS=5e-3)
        ipp2, P_predicted2, pred2, inputs2, ZVS2, ZCS2, penalty2 = obj_func(optimal_x2[None], model_implicit_PINN,P_required, Vin, Vref, with_ZVS=True,modulation="EPS2", return_all=True,threshold_ZVS=5e-3)
        if ipp1 < ipp2:
...
def answer1(ipp,ipp1,model):
    response1 ="The current {} modulation strategy is better than SPS.The conventional SPS realizes {:.2f}A peak-to-peak current at rated power, while showing {:.2f}A peak-to-peak current at 10% light load (100 W).The inductor current waveforms in steady state are shown below. ".format(model,ipp1,ipp)
    return response1
