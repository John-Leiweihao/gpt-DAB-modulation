...
if prompt := st.chat_input("Your question"):  # Prompt for user input and save to chat history
    ...
    if any(keyword in prompt.lower() for keyword in ["consideration","requirement","recommend"]):
    ...
    elif any(keyword in prompt.lower() for keyword in ["PE", "introduce"]):
      with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            response="good"
      ...
      with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            st.session_state.messages.append(message)
    elif any(keyword in prompt.lower() for keyword in ["high", "big", "large","but","seems","good"]):
        with st.chat_message("assistant"):
          with st.spinner("Thinking..."):
              response = chat_engine.chat(prompt, messages_history)
              st.write(response.response)
